...
testCommoditiesTags =
  describe "CommodityTagsParser" $ do
    it "parses a valid commodity line with active" $ do
      ...
      let expected = [CommodityTags "tsmc" "TSMC" True (Just "2330")]
    ...
    it "parses a commodity line with status inactive" $ do
      ...
      let expected = [CommodityTags "dell" "DELL" False Nothing]
      case P.parse commoditiesTagsParser "" (T.pack input) of
        Left err -> expectationFailure (show err)
        Right result -> result `shouldBe` expected
    ...
    it "parses a commodity line without status as active by default" $ do
      ...
      let expected = [CommodityTags "dell" "DELL" True Nothing]
      case P.parse commoditiesTagsParser "" (T.pack input) of
        Left err -> expectationFailure (show err)
        Right result -> result `shouldBe` expected
    ...
    it "parses a commodity in quotes" $ do
      ...
      let expected = [CommodityTags "2330 tw" "2330.TW" True Nothing]
      case P.parse commoditiesTagsParser "" (T.pack input) of
        Left err -> expectationFailure (show err)
        Right result -> result `shouldBe` expected
    ...
    it "parses commodity lines" $ do
      ...
      let expected = [CommodityTags "dell" "DELL" True Nothing, CommodityTags "nvda" "NVDA" True Nothing]
      case P.parse commoditiesTagsParser "" (T.pack input) of
        Left err -> expectationFailure (show err)
        Right result -> result `shouldBe` expected
